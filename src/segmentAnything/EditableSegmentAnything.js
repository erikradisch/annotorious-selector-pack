...
import { toSVGTarget, getPath } from './RubberbandSegmentAnythingTool';
...
export const svgFragmentToPoints = annotation => {
  console.log("svgFragmentToPoints", annotation);

  const svgShape = svgFragmentToShape(annotation);
  console.log("svgFragmentToPoints svgShape", svgShape);

  var polygon = svgShape.getAttribute('d')
  console.log("polygon", polygon);

  var allcoords =  getPointsFromPathValue(polygon)
  console.log("svgFragmentToPoints allcoords", allcoords);

  return allcoords
}

const drawEmbeddedSVG = annotation => {
  console.log("drawEmbeddedSVG", annotation);

  const shape = svgFragmentToShape(annotation);
  console.log("drawEmbeddedSVG shape", shape);

  // Hack
  svgFragmentToPoints(annotation);
  console.log("svgFragmentToPoints done");
...
export default class EditableSegmentAnything extends EditableShape {
  ...
  constructor(annotation, g, config, env) {
    ...
    console.log("constructor editableSegmentAnything this.shape", this.shape);

    this.shape.setAttribute('class', 'a9s-annotation editable selected segment-anything');
    this.elementGroup = document.createElementNS(SVG_NAMESPACE, 'g');
    this.elementGroup.setAttribute('class', 'a9s-annotation editable selected segment-anything');
  ...
  onMouseMove = evt => {
    if (this.grabbedElement) {
      ...
      this.emit('update', toSVGTarget(getPoints(this.shape.firstChild), this.env.image));
    }
  }
  ...
  onScaleChanged = scale => {
    console.log("onScaleChanged editable", scale);
